...
#分appid的
echo `hive -e "use fruit;INSERT OVERWRITE TABLE t_stat_app_uv_tbl PARTITION (stat_date='$yesterday') select appid,count(distinct mid),count(1),'$yesterday' from t_origin_access_stat_tbl where stat_date = '$yesterday' group by appid"`
#总的统计
echo `hive -e "use fruit;INSERT INTO TABLE t_stat_app_uv_tbl PARTITION (stat_date='$yesterday') select -1,count(distinct mid),count(1),'$yesterday' from t_origin_access_stat_tbl where stat_date = '$yesterday'"`
#导入mysql过程
#delete
